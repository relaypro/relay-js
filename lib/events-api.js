...
class RelayEventAdapter extends events.EventEmitter {
  ...
  async start() {
    console.log(`starting event adapter websocket`)
    try {
      this.websocket = await createWebSocket()

      this.websocket.on(`close`, async () => {
        this.websocket = null
        // this.websocket = await createWebSocket() // backoff?
      })
  ...
  async listen(phrases=[], { transcribe=true, timeout=60 }={}) {
    return this.sendReceive(`listen`, { transcribe, phrases, timeout })
  }
...
export const createRelayEventAdapter = async () => {
  const adapater = new RelayEventAdapter()
  await adapater.start()
  return adapater
}
